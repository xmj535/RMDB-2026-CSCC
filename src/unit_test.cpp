...
TEST_F(BufferPoolManagerTest, FlushDirtyPagesOnly) {
    auto disk_manager = BufferPoolManagerTest::disk_manager_.get();
    auto bpm = std::make_unique<BufferPoolManager>(10, disk_manager);
    int fd = BufferPoolManagerTest::fd_;

    PageId dirty_id{.fd = fd, .page_no = INVALID_PAGE_ID};
    PageId clean_id{.fd = fd, .page_no = INVALID_PAGE_ID};
    Page* dirty_page = bpm->new_page(&dirty_id);
    Page* clean_page = bpm->new_page(&clean_id);
    ASSERT_NE(nullptr, dirty_page);
    ASSERT_NE(nullptr, clean_page);

    char zeros[PAGE_SIZE]{};
    disk_manager->write_page(fd, dirty_id.page_no, zeros, PAGE_SIZE);
    disk_manager->write_page(fd, clean_id.page_no, zeros, PAGE_SIZE);
    memcpy(dirty_page->get_data(), "dirty", 6);
    memcpy(clean_page->get_data(), "clean-but-not-dirty", 20);
    ASSERT_TRUE(bpm->unpin_page(dirty_id, true));
    ASSERT_TRUE(bpm->unpin_page(clean_id, false));

    EXPECT_EQ(1U, bpm->flush_dirty_pages(fd));
    EXPECT_FALSE(dirty_page->is_dirty());

    char dirty_disk[PAGE_SIZE]{};
    char clean_disk[PAGE_SIZE]{};
    disk_manager->read_page(fd, dirty_id.page_no, dirty_disk, PAGE_SIZE);
    disk_manager->read_page(fd, clean_id.page_no, clean_disk, PAGE_SIZE);
    EXPECT_STREQ("dirty", dirty_disk);
    EXPECT_EQ(0, memcmp(clean_disk, zeros, PAGE_SIZE));

    // 已刷脏页不会在下一次调用被重复写回。
    EXPECT_EQ(0U, bpm->flush_dirty_pages(fd));
}
