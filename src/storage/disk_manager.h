...
class DiskManager {
   public:
    ...
    /*日志操作*/
    int64_t read_log(char *log_data, int64_t size, int64_t offset);

    void write_log(char *log_data, int size);

    // Make all preceding writes to db.log durable before a COMMIT can be
    // acknowledged. The descriptor is the same object used by write_log().
    void sync_log();
