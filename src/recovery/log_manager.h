...
public:
    ...
    virtual void deserialize(const char* src) {
        log_type_ = log_serialization::read_value<LogType>(src + OFFSET_LOG_TYPE);
        lsn_ = log_serialization::read_value<lsn_t>(src + OFFSET_LSN);
        log_tot_len_ =
            log_serialization::read_value<uint32_t>(src + OFFSET_LOG_TOT_LEN);
        log_tid_ =
            log_serialization::read_value<txn_id_t>(src + OFFSET_LOG_TID);
        prev_lsn_ =
            log_serialization::read_value<lsn_t>(src + OFFSET_PREV_LSN);
    }
    // used for debug
...
public:
    ...
    void deserialize(const char* src) override { LogRecord::deserialize(src); }
    void format_print() override {
        printf("static checkpoint record\n");
        LogRecord::format_print();
    }
};
...
public:
    ...
    void deserialize(const char* src) override {
        ...
        rid_ = log_serialization::read_value<Rid>(src + offset);
        offset += sizeof(Rid);
        table_name_size_ = log_serialization::read_value<size_t>(src + offset);
        offset += sizeof(size_t);
        table_name_ = new char[table_name_size_];
...
/* delete操作的日志记录：记录被删行的旧值、位置、表名（undo 用旧值复原，redo 删除） */
...
public:
    ...
    void deserialize(const char* src) override {
        ...
        table_name_size_ = log_serialization::read_value<size_t>(src + offset);
        offset += sizeof(size_t);
        if (table_name_ != nullptr) delete[] table_name_;
        table_name_ = new char[table_name_size_];
        memcpy(table_name_, src + offset, table_name_size_);
    }
    void format_print() override {
    ...
    RmRecord delete_value_;     // 被删除的记录（旧值）
    Rid rid_;                   // 记录被删除的位置
    char* table_name_;          // 表名称
    size_t table_name_size_;    // 表名称的大小
};

/* update操作的日志记录：记录旧值与新值、位置、表名（undo 用旧值，redo 用新值） */
...
public:
    ...
    UpdateLogRecord(txn_id_t txn_id, RmRecord& old_value, RmRecord& new_value,
                    Rid& rid, std::string table_name)
        : UpdateLogRecord() {
        log_tid_ = txn_id;
...
public:
    ...
    // Group commit: 把已进入 buffer 的日志批量刷盘。
    // 调用方先 add_log_to_buffer 获得 lsn，再调此方法等待该 lsn 落盘。
    // 多个并发提交线程共享同一次 fsync：第一个成为 leader 执行刷盘，
    // 后续 follower 等待 leader 完成后直接返回。
    void group_commit(lsn_t my_lsn);

    LogBuffer* get_log_buffer() { return &log_buffer_; }

    // 恢复后把全局 lsn 推到日志中最大 lsn 之后，保证新日志号单调
    void set_global_lsn(lsn_t lsn) { global_lsn_.store(lsn); }
    lsn_t get_global_lsn() { return global_lsn_.load(); }

private:
    // flush 的内部实现，假设调用方已持有 latch_
    ...
    // 当前缓冲区内已序列化的最大 lsn。刷盘后 persist_lsn_ 取这个值，
    // 保证 persist_lsn_ 只覆盖真正写出去的记录（见 flush_log_to_disk_unlocked）。
    lsn_t buffer_max_lsn_{INVALID_LSN};
