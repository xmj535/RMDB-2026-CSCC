  ...
  std::shared_ptr<Plan> plan_query(std::shared_ptr<Query> query,
    ...
    if (auto x = std::dynamic_pointer_cast<ast::Help>(query->parse)) {
      ...
      return std::make_shared<OtherPlan>(T_Help, std::string());
                   ...
                   std::dynamic_pointer_cast<ast::ShowTables>(query->parse)) {
      // show tables;
      return std::make_shared<OtherPlan>(T_ShowTable, std::string());
    } else if (auto x =
                   std::dynamic_pointer_cast<ast::ShowIndex>(query->parse)) {
      // show index from tab;
      return std::make_shared<OtherPlan>(T_ShowIndex, x->tab_name);
    } else if (auto x =
                   std::dynamic_pointer_cast<ast::DescTable>(query->parse)) {
      // desc table;
    ...
    } else if (auto x = std::dynamic_pointer_cast<ast::SetIsolationStmt>(
                   query->parse)) {
      IsolationLevel iso = (x->level == ast::SV_ISO_SNAPSHOT_ISOLATION)
                               ? IsolationLevel::SNAPSHOT_ISOLATION
                               : IsolationLevel::SERIALIZABLE;
      return std::make_shared<SetIsolationPlan>(iso);
    } else if (auto x = std::dynamic_pointer_cast<ast::LoadStmt>(query->parse)) {
      return std::make_shared<LoadPlan>(x->file_path, x->tab_name);
    } else {
