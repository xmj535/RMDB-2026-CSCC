...
private:
    ...
    static void print_node(const std::shared_ptr<TreeNode> &node, int offset) {
        ...
        if (auto x = std::dynamic_pointer_cast<Help>(node)) {
            ...
            std::cout << "STRING_LIT\n";
            print_val(x->val, offset);
        } else if (auto x = std::dynamic_pointer_cast<ParamRef>(node)) {
            std::cout << "PARAM_REF\n";
            print_val("$" + x->ordinal, offset);
        } else if (auto x = std::dynamic_pointer_cast<SetClause>(node)) {
            std::cout << "SET_CLAUSE\n";
            print_val(x->col_name, offset);
            if (x->rhs_is_col) {
                print_val(x->rhs_col, offset);
            } else if (x->op != 0) {
                print_val(x->rhs_col, offset);
                print_val(std::string(1, x->op), offset);
                print_node(x->val, offset);
                for (const auto &term : x->trailing_terms) {
                    print_val(std::string(1, term.op), offset);
